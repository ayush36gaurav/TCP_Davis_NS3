...
std::ofstream cWndLog("cWnd-log.csv");

void LogCwnd(uint32_t oldCwnd, uint32_t newCwnd, uint32_t senderId)
{
    cWndLog << Simulator::Now().GetSeconds() << "," << senderId << "," << newCwnd << std::endl;
}

void ConnectCwndTrace(Ptr<Socket> socket, uint32_t senderId)
{
    Ptr<TcpSocketBase> tcpSocket = DynamicCast<TcpSocketBase>(socket);
    if(tcpSocket)
    {
        tcpSocket->TraceConnectWithoutContext("CongestionWindow", MakeBoundCallback(&LogCwnd, senderId));
    }
}
...
void CalculateThroughput(Ptr<FlowMonitor> flowMonitor, FlowMonitorHelper &flowHelper, double interval, double &time, std::map<FlowId, uint64_t> &lastRxBytes)
{
    ...
    for(auto it = stats.begin(); it!=stats.end(); ++it)
    {
        ...
        // else
        // {
        //     throughput = ((currentRxBytes) * 8.0) / (interval * 1e6);
        // }
...
int main(int argc, char *argv[])
{
    ...
    Ptr<BulkSendApplication> bulkSendApp0 = DynamicCast<BulkSendApplication>(sourceApp0.Get(0));
    if(bulkSendApp0)
    {
        Ptr<Socket> socket0 = bulkSendApp0->GetSocket();
        ConnectCwndTrace(socket0, 0);
    }
    ...
    Ptr<BulkSendApplication> bulkSendApp1 = DynamicCast<BulkSendApplication>(sourceApp1.Get(0));
    if(bulkSendApp1)
    {
        Ptr<Socket> socket1 = bulkSendApp1->GetSocket();
        ConnectCwndTrace(socket1, 1 );
    }
